...
class FFM(nn.Module):
	def __init__(self, feat_columns, embed_dim, k):
		...
		feature_num = len(dense_feat_columns) + len(sparse_feat_columns) * embed_dim
		field_num = len(dense_feat_columns) + len(sparse_feat_columns)
		self.field_num = field_num
		self.w = nn.Linear(feature_num, 1, bias=True)
		self.v = nn.Parameter(torch.rand(feature_num, field_num, k))
	...
	def forward(self, dense_input, sparse_input):
		...
		# linear
		first_ffm = self.w(all_input).squeeze()
		# ffm
		# 每个特征值和自己的(field_num, k)相乘计算 v(i,fj,k)xi
		# (batch_size, feature_num), (feature_num, field_num, k) -> (batch_size, field_num, k)
		second_ffm = 0
		field_f = torch.tensordot(all_input, self.v, dims=1)
		for i in range(self.field_num):
			for j in range(i+1, self.field_num):
				second_ffm += torch.sum(torch.mul(field_f[:, i, :], field_f[:, j, :]), dim=1)
		output = self.sigmoid(first_ffm + second_ffm)
		return output
...
if __name__ == '__main__':
	...
	model = FFM(feat_columns, embed_dim, k)
	model.to(device)
